...
用自然语言分析 MF4 信号数据。输入一句话，自动提取信号、检测边沿、生成可视化图片。

## 安装

```bash
pip install streamlit==1.44.0 langgraph openai python-dotenv pyyaml asammdf matplotlib numpy
```
...
```bash
# 单信号
python demo.py measure_USS_04.mf4 EPS_StgTq.Val 10 30

# 多信号（叠加）
python demo.py measure_USS_04.mf4 EPS_StgTq.Val LWI_AgStgWhl.Val 10 30

# 多信号（分栏）
python demo.py measure_USS_04.mf4 EPS_StgTq.Val LWI_AgStgWhl.Val 10 30 --split
```
